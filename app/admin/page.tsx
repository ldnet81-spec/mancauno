...
export default async function AdminPage({ searchParams }: AdminPageProps) {
  ...
  const { data: users } = await supabase
    .from("profiles")
    .select("id, display_name, city, role, banned_at, created_at")
    ...
    <main className="mx-auto min-h-screen max-w-5xl px-6 py-8">
      <div className="mb-8">
        <div className="flex items-center justify-between gap-4">
          ...
          <Link
            href="/profilo"
            className="rounded-full border border-gray-200 px-4 py-2 text-sm font-medium text-black"
          >
            Profilo
          </Link>
        </div>
                ...
                <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-500">
                      Status: {event.status} · Mancano {event.remaining_spots} · Coda {event.waitlisted_count ?? 0}
                    </p>
